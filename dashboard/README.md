...
This local React dashboard turns the latest CrisisPulse Parquet and JSON outputs into a readable evidence screen. It shows candidate counts, scoring readiness, supported regional signals, data coverage, and a human-review queue with direct publisher evidence and the guardrails behind every decision.
...
The dashboard requires no paid API. Evidence is grouped by duplicate-adjusted story ID and links directly to the retained publisher URLs. Because GDELT does not provide verified headlines in this feed, the dashboard says when a readable label was derived from a URL path. A candidate represents unusual news reporting that needs human review; even a saved `Real event` label is evaluation data, not a public warning or proof issued by CrisisPulse.
